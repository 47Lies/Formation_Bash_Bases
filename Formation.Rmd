...
**Bash** is the most frequent command line interpretor in linux distributions.

It will allow you tu use most of the bioinformatics tools, which are mainly command-line tools.
...
They is several norms for naming variables but they might not be respected. We will recommend you to:
...
--> J'ai pas compris ce que tu voulais dire ni pourquoi tu utilises --time-style=full-iso
...
The right for owner, group and other are displayed in the first columns 3 to 5. If you have a right, the corresponding letter is displayed if not, an hyphen is displayed.
...
In some occasions, it can be more convenient to store instructions (commands) in a file and executes these instructions in one time. A file with bash instructions is then called a bash script. By convention, it has a `.sh` extension. In that script file, each instruction is on a new line. They will be executed sequentially (one by one, from top to bottom) and each instruction will have to finish before going to the next one.
